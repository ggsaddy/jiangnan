...
class all_components:
    ...
    def update_zitu(self,zitu_list:list):
        indexs=[item for sublist in zitu_list for item in sublist[1]]
        try:
            for index in sorted(indexs,reverse=True):
                del self.uf.parent[index]
        except:
            print(len(indexs))
            print(len(self.uf.parent))
            print(self.uf.parent)
        self.update_groups()
