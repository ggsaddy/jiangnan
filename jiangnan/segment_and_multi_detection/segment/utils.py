...
from segment_and_multi_detection.segment.src import component, UnionFind, all_components, sub_components
...
def load_data_and_get_main_bbox(filepath, threshold=0.1):
    ...
    return result
#OLD
def load_data_and_get_main_bbox_boound(filepath, threshold=0.1,bound_json=None):
    ...
    with open(filepath, 'r', encoding='utf8') as fp:
        ...
        for data in json_data_main:
            ...
            if is_fully_contained_strictly(bound_data, data["bound"]):
                area = calculate_area(data["bound"])
                if area >= area_threshold * 0.99 and area < area_threshold * 20:
                    main_bbox_dict[count] = data["bound"]
                    count += 1
