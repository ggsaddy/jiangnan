...
def polar_point_v2(center, radius, angle_rad):
    # angle_rad = math.radians(angle_deg)
    x = center[0] + radius * math.cos(angle_rad)
    y = center[1] + radius * math.sin(angle_rad)
    return [x, y]
...
def convertArc(doc, entity:ezdxf.entities.ArcEdge):
    ...
    middle_angle = (start_angle + delta / 2) % 360

    mid['start_point'] = polar_point(center, radius, start_angle)
    mid['end_point'] = polar_point(center, radius, end_angle)
    mid['mid_point'] = polar_point(center, radius, middle_angle)
...
def convertLWPolyline(doc, entity:ezdxf.entities.Spline):
    ...
    for i in range(entity.dxf.count  - 1):
        ...
        if start_point[-1] != 0.:
            ...
            middle_angle = (start_angle + delta / 2) % (2 * math.pi)
            # 计算起点、终点和中间点
            start_pt = polar_point_v2(center, radius, start_angle)
            end_pt = polar_point_v2(center, radius, end_angle)
            mid_pt = polar_point_v2(center, radius, middle_angle)
            
            mid["vertices"].append([x, y, start_angle, end_angle, radius, start_pt, end_pt, mid_pt])
    ...
    if entity.is_closed:
        ...
        if start_point[-1] != 0.:
            ...
            # 添加计算出的点位信息
            if 'arcPoints' not in mid:
                mid['arcPoints'] = []
            mid['arcPoints'].append({
