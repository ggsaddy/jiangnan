...
import math
from PanelInfoExtration.plot_geo import plot_geometry,plot_polys, plot_info_poly
...
from PanelInfoExtration.bracket_parameter_extraction import *
from shapely.geometry import Polygon,Point
import shutil
from datetime import datetime
import csv

# def save_polys_as_csv(polys, filename):
#         with open(filename, 'w', newline='') as f:
#             writer = csv.writer(f)
#             writer.writerow(['poly_id', 'point_order', 'x', 'y'])
        
#             for i, poly in enumerate(polys):
#                 for j, point in enumerate(poly):
#                     writer.writerow([i, j, point.x, point.y])
#                 # 添加空行分隔不同多边形
#                 writer.writerow([])
...
def readJson(path,segmentation_config):
    ...
    # segments=[]
    # color = [3, 7, 8, 4,2,140]
    # linetype = ["BYLAYER", "Continuous","Bylaye
    # r","CONTINUOUS","ByBlock","BYBLOCK"]
...
# def findPolys_via_excel(panel_info, segments_info, elements,texts,dimensions,segments,sign_handles,segmentation_config)
...
def findClosedPolys_via_BFS(elements,texts,dimensions,segments,sign_handles,segmentation_config):
    ...
    cornor_holes = filter_cornor_holes(cornor_holes,filtered_point_map,segmentation_config)        ####到这里有用####
    # for cornor_hole in cornor_holes:
    #     for s in cornor_hole.segments:
    #         s.isCornerhole=True
